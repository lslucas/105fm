<?php
   ...
   */
  $sql_letras = "SELECT UPPER(LEFT(fot_titulo, 1)) FROM ".TABLE_PREFIX."_{$var['table']} WHERE 1 GROUP BY LEFT(fot_titulo, 1) ORDER BY fot_titulo";
...
$sql = "SELECT  ${var['pre']}_id,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $code, $titulo, $status, $dt);


    if($total_itens==0) $total = 'Nenhum album'.$countLetra;
    elseif ($total_itens==1) $total = "1 album".$countLetra;
	else $total = $total_itens.' albums'.$countLetra;

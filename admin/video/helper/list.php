<?php
   ...
   */
  $sql_letras = "SELECT UPPER(LEFT(vid_titulo, 1)) FROM ".TABLE_PREFIX."_{$var['table']} WHERE 1 GROUP BY LEFT(vid_titulo, 1) ORDER BY vid_titulo";
  ...
  if($qry_letras = $conn->prepare($sql_letras)) {
    ...
    $countLetra = '';
      ...
      $letras .= "<a class='btn btn-mini' href='?p=vidica'>Todos</a>";
      ...
      while($qry_letras->fetch()) {
        if(!isset($_GET['letra']) || $letra<>$_GET['letra'])
	        $letras .= "<a class='btn btn-mini' href='?p=vidica&letra=${letra}'>";
		else
...
$sql = "SELECT  ${var['pre']}_id,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $code, $artista, $titulo, $data, $album, $youtube, $status, $destaque, $principal, $dt_cadastro);


    if($total_itens==0) $total = 'Nenhum vidica'.$countLetra;
    elseif ($total_itens==1) $total = "1 vidica".$countLetra;
	else $total = $total_itens.' vidicas'.$countLetra;

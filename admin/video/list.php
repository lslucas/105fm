...
<div align=left>
		<form name='search' action='<?=$_SERVER['PHP_SELF']?>' method='get' class='form form-horizontal'>
			...
			<input type='text' name='q' class='input-large' placeholder='Título ou Artista' value='<?=isset($_GET['q']) ? $_GET['q'] : null?>'>
...
<table class="table table-condensed table-striped">
   <thead>
      <tr>
        ...
        <th width="60px" title='Vídeo Destaque na Home' class='tip'>Destaque?</th>
        <th width="60px" title='Vídeo Principal' class='tip'>Principal?</th>
      </tr>
   </thead>
   <tbody>
<?php
...
?>
	...
	<tr id="tr<?=$id?>">
		...
		<td align=center>
			<a href="?p=<?=$p?>&destaque&item=<?=$id?>&noVisual" class='tip destaque destaque<?=$id?>' title='Clique para alterar para destaque ou não destaque' id='<?=$id?>' name='<?=$titulo?>'><?=$destaque==1?'Sim':'Não'?></a>
		</td>
		<td align=center>
			<a href="?p=<?=$p?>&principal&item=<?=$id?>&noVisual" class='tip principal principal<?=$id?>' title='Clique para alterar para vídeo principal ou não principal' id='<?=$id?>' name='<?=$titulo?>'><?=$principal==1?'Sim':'Não'?></a>
		</td>
	</tr>
<?php
     $j++;

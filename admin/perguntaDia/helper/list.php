...
$sql = "SELECT  ${var['pre']}_id,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $titulo, $status, $data, $timestamp);

    if($total_itens==0) $total = 'Nenhuma enquete'.$countLetra;
    elseif ($total_itens==1) $total = "1 enquete".$countLetra;
    else $total = $total_itens.' enquetes'.$countLetra;

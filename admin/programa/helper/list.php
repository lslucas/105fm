<?php
  ...
  if($qry_letras = $conn->prepare($sql_letras)) {
    ...
    $countLetra = '';
      ...
      $letras .= "<a class='btn btn-mini' href='?p=programa'>Todos</a>";
      ...
      while($qry_letras->fetch()) {
        if(!isset($_GET['letra']) || $letra<>$_GET['letra'])
	        $letras .= "<a class='btn btn-mini' href='?p=programa&letra=${letra}'>";
		else
  ...
  if( isset($_GET['q']) && !empty($_GET['q']) ) {
	...
	$where.= " OR ${var['pre']}_apresentacao LIKE '%".$_GET['q']."%' ";
	$where.= ")";
  }
...
$sql = "SELECT  ${var['pre']}_id,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $titulo, $apresentacao, $status, $dt_cadastro);


    if($total_itens==0) $total = 'Nenhum programa'.$countLetra;
    elseif ($total_itens==1) $total = "1 programa".$countLetra;
	else $total = $total_itens.' programas'.$countLetra;

<?php
   ...
   */
  $sql_letras = "SELECT UPPER(LEFT({$var['pre']}_times, 1)) FROM ".TABLE_PREFIX."_{$var['table']} WHERE 1 GROUP BY LEFT({$var['pre']}_times, 1) ORDER BY {$var['pre']}_times";
  ...
  $join = null;
  if( isset($_GET['letra']) && !empty($_GET['letra']) ) {
    $where.= " AND ${var['pre']}_times LIKE '".$_GET['letra']."%' ";
  }

  if( isset($_GET['q']) && !empty($_GET['q']) ) {
	$where.= " AND ( ";
	$where.= " ${var['pre']}_times LIKE '%".$_GET['q']."%' ";
...
$orderby = !isset($_GET['orderby'])?$var['pre'].'_times ASC':urldecode($_GET['orderby']);
...
$sql = "SELECT  ${var['pre']}_id,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $times, $horario, $apresentador, $status, $data);

    if($total_itens==0) $total = 'Nenhuma partido'.$countLetra;
    elseif ($total_itens==1) $total = "1 partido".$countLetra;
    else $total = $total_itens.' partidos'.$countLetra;

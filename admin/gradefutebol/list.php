	...
	<div style='float:right; width:90px; text-align:right; '>
		...
		<div class="btn-group">
		  ...
		  <ul class="dropdown-menu">
			...
			<li><a href="?p=<?=$p.$pag.$letter?>&orderby=<?=$var['pre'].'_times'?> ASC"<?php if($orderby==$var['pre'].'_times ASC') echo ' selected';?>">Nome Crescente</a></li>
			<li><a href="?p=<?=$p.$pag.$letter?>&orderby=<?=$var['pre'].'_times'?> DESC"<?php if($orderby==$var['pre'].'_times DESC') echo ' selected';?>">Nome Decrescente</a></li>
		  </ul>
		</div>
...
<?php
	if ($total_itens==0)
		echo "<div class='alert alert-info'>Nenhuma partido!</div>";
...
$row_actions = <<<end
		<div class="btn-group">
          ...
          <ul class="dropdown-menu">
			...
			<li><a href="?p=$p&status&item=$id&noVisual" class='tip status status$id'  alt='{$altStatus}' title='Clique para alterar o status do ítem selecionado' id='$id' name='$times'>{$descStatus}</a></li>
          </ul>
		</div>
end;
...
?>
	<div class="modal fade" id="rm-modal<?=$id?>">
		...
		<p>Deseja remover <b><?=$times?></b>?<div class='alert alert-warning small'>Ele será removido permanentemente!</div></p>
		</div>

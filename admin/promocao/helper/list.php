<?php
   ...
   */
  $sql_letras = "SELECT UPPER(LEFT(pro_titulo, 1)) FROM ".TABLE_PREFIX."_{$var['table']} WHERE 1 GROUP BY LEFT(pro_titulo, 1) ORDER BY pro_titulo";
  ...
  if($qry_letras = $conn->prepare($sql_letras)) {
    ...
    $countLetra = '';
      ...
      $letras .= "<a class='btn btn-mini' href='?p=pro'>Todos</a>";
      ...
      while($qry_letras->fetch()) {
        if(!isset($_GET['letra']) || $letra<>$_GET['letra'])
	        $letras .= "<a class='btn btn-mini' href='?p=pro&letra=${letra}'>";
		else
...
$sql = "SELECT  ${var['pre']}_id,
		...
		${var['pre']}_titulo,
           ${var['pre']}_linhafina,
		DATE_FORMAT(${var['pre']}_data_inicio, '%d/%m/%Y') data,
		DATE_FORMAT(${var['pre']}_data_termino, '%d/%m/%Y') hora_inicio,
		${var['pre']}_status,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $code, $titulo, $linhafina, $data_inicio, $data_termino, $status, $timestamp);


    if($total_itens==0) $total = 'Nenhuma promoção'.$countLetra;
    elseif ($total_itens==1) $total = "1 promoção".$countLetra;
	else $total = $total_itens.' promoções'.$countLetra;

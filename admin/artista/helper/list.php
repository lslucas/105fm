<?php
   ...
   */
  $sql_letras = "SELECT UPPER(LEFT(art_titulo, 1)) FROM ".TABLE_PREFIX."_{$var['table']} WHERE 1 GROUP BY LEFT(art_titulo, 1) ORDER BY art_titulo";
  ...
  if($qry_letras = $conn->prepare($sql_letras)) {
    ...
    $countLetra = '';
      ...
      $letras .= "<a class='btn btn-mini' href='?p=artigo'>Todos</a>";
      ...
      while($qry_letras->fetch()) {
        if(!isset($_GET['letra']) || $letra<>$_GET['letra'])
	        $letras .= "<a class='btn btn-mini' href='?p=artigo&letra=${letra}'>";
		else
...
$sql = "SELECT  ${var['pre']}_id,
 ...
 if (!$qry = $conn->prepare($sql)) {
  echo 'Houve algum erro durante a execução da consulta<p class="code">'.$sql.'</p><hr>';
    ...
    $qry->execute();
    $qry->bind_result($id, $code, $titulo, $site, $facebook, $youtube, $twitter, $status, $dt_cadastro);


    if($total_itens==0) $total = 'Nenhum artista'.$countLetra;
    elseif ($total_itens==1) $total = "1 artista".$countLetra;
	else $total = $total_itens.' artistas'.$countLetra;

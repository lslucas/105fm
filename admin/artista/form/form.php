 <div class='alert alert-error completeform-error hide'>
	...
	<ol>
		...
		<li><label for="site" class="error-validate">Informe uma url válida para o <b>site</b></label></li>
		<li><label for="facebook" class="error-validate">Informe uma url válida para o <b>facebook</b></label></li>
		<li><label for="twitter" class="error-validate">Informe uma url válida para o <b>twitter</b></label></li>
...
<p class='header'>Todos os campos com <b>- * -</b> são obrigatórios.</p>
  ...
  <fieldset>
      ...
      <div class="controls">
    	  <?php
    	    ...
    	    if ($act=='update') {
    		    ...
    		    $sql_gal = "SELECT rag_id, rag_imagem, rag_pos FROM ".TABLE_PREFIX."_${var['table']}_galeria WHERE rag_{$var['pre']}_id=? AND rag_imagem IS NOT NULL ORDER BY rag_pos ASC LIMIT 1;";
    	  ...
    	  ?>
    		<tr id="<?=$g_id?>">
    		  ...
    		  <td>
				<small>
    		    [<a href='?p=<?=$p?>&delete_galeria&item=<?=$g_id?>&prefix=<?=$var['table']?>_galeria&pre=rag&col=imagem&folder=<?=$var['imagem_folderlist']?>&noVisual' title="Clique para remover o ítem selecionado" class='tip trash-galeria' style="cursor:pointer;" id="<?=$g_id?>">remover</a>]
				</small>
    		  </td>
    		  <td>
    ...
    <div class="control-group">
      <label class="control-label" for="titulo">* Nome do Artista</label>
      <div class="controls">
    ...
    <div class="control-group">
      <label class="control-label" for="youtube">Canal do Youtube</label>
      <div class="controls">
        <input type="text" class="input-xlarge url" placeholder='URL do Canal do Youtube' name='youtube' id='youtube' value='<?=$val['youtube']?>'>
      </div>
    </div>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <input type="text" class="input-xlarge url" placeholder='Twitter' name='twitter' id='twitter' value='<?=$val['twitter']?>'>
      </div>
    </div>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <input type="text" class="input-xlarge url" placeholder='Facebook' name='facebook' id='facebook' value='<?=$val['facebook']?>'>
      </div>
    </div>

    <div class="control-group">
      <label class="control-label" for="site">Site Oficial</label>
      <div class="controls">
        <input type="text" class="input-xlarge url" placeholder='http://artista.com.br' name='site' id='site' value='<?=$val['site']?>'>
      </div>
    </div>
